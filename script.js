...
document.querySelector('.check').addEventListener('click',()=>{
    ...
    //message d'erreur
    function msgErrorNotIncluNumber(){
        document.querySelector('.message').textContent="Number is not betwwen 1 and 20!!!";
         document.querySelector('.number').textContent = '?'; // afficher le nombre exact (le nombre récupéré de l'input)
    } 

    if(valueInputNumber < 1){
        msgErrorNotIncluNumber();
    }
        ...
        if(valueInputNumber !== guessNumber){
            document.querySelector('.message').textContent = valueInputNumber > guessNumber ? "Your Number is Low" : "Your Number is Less";
            score = score - 1;
